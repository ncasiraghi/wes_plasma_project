...
setwd("/elaborazioni/sharedCO/Abemus_data_analysis/NimbleGen_SeqCapEZ_Exome_v3_Plasma_IPM/Mutect_mutation_load_tables")

toolName <- "Mutect"

tissue_haloplex_calls <- "/elaborazioni/sharedCO/Abemus_data_analysis/NimbleGen_SeqCapEZ_Exome_v3_Plasma_IPM/Mutect/Tissue_HaloPlex/calls_mutect.txt"

tissue_sureselect_calls <- "/elaborazioni/sharedCO/Abemus_data_analysis/NimbleGen_SeqCapEZ_Exome_v3_Plasma_IPM/Mutect/Tissue_SureSelect/calls_mutect.txt"

plasma_nimble_calls <- "/elaborazioni/sharedCO/Abemus_data_analysis/NimbleGen_SeqCapEZ_Exome_v3_Plasma_IPM/Mutect/Plasma/calls_mutect.txt"
...
for(indx in seq( 1 ,nrow(filter))){
  ...
  # Tissue HaloPlex  
  halo.calls = readLines(con = tissue_haloplex_calls)
  
  halo.calls = data.frame(id.patient = str_split_fixed(halo.calls, "/", 13)[,11], calls = halo.calls, stringsAsFactors = F)
  
  # Tissue SureSelect
  sure.calls = readLines(con = tissue_sureselect_calls)
  
  sure.calls = data.frame(id.patient = str_split_fixed(sure.calls, "/", 13)[,11], calls = sure.calls, stringsAsFactors = F)
  
  # Plasma
  calls = readLines(con = plasma_nimble_calls)
  
  plasma.calls = data.frame(id.case = str_split_fixed(calls, "/", 13)[,11], calls = calls, stringsAsFactors = F)
